...
public class MainController {
    ...
    private void handleAddUser() {
        try {
            ...
            Stage dialogStage = new Stage();
            dialogStage.setTitle("Добавление пользователя");
            dialogStage.initModality(Modality.WINDOW_MODAL);
            ...
            showError("Ошибка", "Не удалось открыть форму добавления пользователя: " + e.getMessage());
        }
    }
    ...
    private void handleEditUser() {
        ...
        if (selected == null) {
            showError("Изменение пользователя", "Выберите пользователя для изменения");
            return;
        }
        ...
        try {
            ...
            Stage dialogStage = new Stage();
            dialogStage.setTitle("Изменение пользователя");
            dialogStage.initModality(Modality.WINDOW_MODAL);
            ...
            if (controller.isSaveClicked()) {
                ...
                apiService.updateUser(selected.getId(), updatedUser);
                refreshUsers();
            }
        } catch (IOException e) {
            showError("Ошибка", "Не удалось открыть форму изменения пользователя: " + e.getMessage());
        }
    }
